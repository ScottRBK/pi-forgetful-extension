...
export function validateKnowledgeWriteRequest(value: unknown): KnowledgeWriteRequest {
  ...
  switch (op) {
    ...
    case "create_entity":
      ...
      if (!ENTITY_TYPES.has(String(input.entity_type))) throw new Error("entity_type is invalid.");
    ...
    case "update_entity":
      ...
      if (input.entity_type !== undefined && !ENTITY_TYPES.has(String(input.entity_type)))
...
export async function executeKnowledgeRead(
  ...
  const requireKnowledge = (): KnowledgeClient => {
    if (!knowledge) throw new Error("Rich Forgetful knowledge is unavailable.");
    return knowledge;
  };
  const projectId = context.scope === "project" ? currentProject(context) : undefined;
  switch (request.operation) {
    case "list_projects": {
      const repoName = request.repo_name as string | undefined;
      if (context.scope === "project" && repoName && repoName !== context.repoName)
        throw new Error("Project reads must stay inside the current repository.");
      let projects = await client.listProjects(repoName ?? context.repoName, signal);
      if (projectId !== undefined) projects = projects.filter((item) => item.id === projectId);
      const result = boundedListPage(
        projects, request.offset as number | undefined, request.limit as number | undefined,
      );
      return readResult(result, { operation: request.operation, count: result.items.length });
    }
    case "search_memories": {
      const requestedProject = context.scope === "project"
        ? currentProject(context, request.project_id as number | undefined)
        : request.project_id as number | undefined;
      let values = await client.search({
        query: request.query as string,
        query_context: (request.query_context as string | undefined) ?? "Foreground knowledge read",
        strict_project_filter: context.scope === "project" || requestedProject !== undefined,
        ...(requestedProject === undefined ? {} : { project_ids: [requestedProject] }),
        k: (request.k as number | undefined) ?? 10,
        include_links: (request.include_links as boolean | undefined) ?? true,
      }, signal);
      if (requestedProject !== undefined)
        values = values.filter((item) => inMemoryProject(item, requestedProject));
      return readResult(values, { operation: request.operation, count: values.length });
    }
    case "get_memory": {
      const value = await assertReadMemory(client, request.memory_id as number, context, signal);
      return readResult(value, { operation: request.operation, id: value.id });
    }
    case "search_entities": {
      const requestedLimit = (request.limit as number | undefined) ?? 10;
      const requestedOffset = (request.offset as number | undefined) ?? 0;
      const searchLimit = MAX_RECORD_LIMIT;
      let values = await requireKnowledge().searchEntities(
        request.query as string, searchLimit, signal,
      );
      const backendCount = values.length;
      values = await hydrateEntities(
        requireKnowledge(), values.slice(0, searchLimit), signal,
      );
      const entityProjectId = context.scope === "project"
        ? currentProject(context, request.project_id as number | undefined)
        : request.project_id as number | undefined;
    ...
    case "get_entity": {
      const value = await assertReadEntity(
        requireKnowledge(), request.entity_id as number, context, signal,
      );
      return readResult(value, { operation: request.operation, id: value.id });
    }
    case "get_entity_memories": {
      await assertReadEntity(
        requireKnowledge(), request.entity_id as number, context, signal,
      );
      const linked = await requireKnowledge().getEntityMemories(
        request.entity_id as number, signal,
      );
      const linkedPage = listPage(
        linked, request.offset as number | undefined, request.limit as number | undefined,
      );
      const checked = await Promise.all(linkedPage.items.map(async (item) => {
        try {
          const memory = await assertReadMemory(client, item.id, context, signal);
      ...
      return readResult({
        ...
        operation: request.operation, count: values.length,
      });
    }
    case "get_relationships": {
      ...
      const relationships = await requireKnowledge().getRelationships(entity.id, signal);
      const relationshipPage = listPage(
        relationships,
        request.offset as number | undefined,
        request.limit as number | undefined,
      );
      ...
      const endpoints = await Promise.all(endpointIds.map(async (id) => {
        try {
          return await assertReadEntity(requireKnowledge(), id, context, signal);
        } catch {
          return undefined;
        }
      }));
    ...
    case "list_documents": {
      let values = await requireKnowledge().listDocuments(
        context.scope === "project"
          ? currentProject(context, request.project_id as number | undefined)
          : request.project_id as number | undefined,
        signal,
      );
      if (projectId !== undefined) values = values.filter((item) => item.project_id === projectId);
      const result = boundedListPage(
        values, request.offset as number | undefined, request.limit as number | undefined,
      );
      return readResult(result, { operation: request.operation, count: result.items.length });
    }
    case "get_document": {
      const value = await requireKnowledge().getDocument(request.document_id as number, signal);
      if (projectId !== undefined && value.project_id !== projectId)
        throw new Error("The document is outside the current project.");
      const result = page(value, "content", request.offset as number | undefined,
        request.limit as number | undefined);
      return readResult(result, { operation: request.operation, id: value.id });
    }
    case "list_code_artifacts": {
      let values = await requireKnowledge().listCodeArtifacts(
        context.scope === "project"
          ? currentProject(context, request.project_id as number | undefined)
          : request.project_id as number | undefined,
        signal,
    ...
    case "get_code_artifact": {
      const value = await requireKnowledge().getCodeArtifact(
        request.code_artifact_id as number, signal,
      );
      if (projectId !== undefined && value.project_id !== projectId)
        throw new Error("The code artifact is outside the current project.");
      const result = page(value, "code", request.offset as number | undefined,
        request.limit as number | undefined);
      return readResult(result, { operation: request.operation, id: value.id });
    }
    case "list_files": {
      let values = await requireKnowledge().listFiles(
        context.scope === "project"
          ? currentProject(context, request.project_id as number | undefined)
          : request.project_id as number | undefined,
        signal,
    ...
    case "get_file": {
      const value = await requireKnowledge().getFile(request.file_id as number, signal);
      if (projectId !== undefined && value.project_id !== projectId)
      ...
      return {
        ...
        details: { operation: request.operation, id: value.id, kind: rendered.kind },
        file: value,
      };
    }
  }
...
function memoryState(memory: Memory): string {
  return JSON.stringify({
    ...
    source_files: [...(memory.source_files ?? [])].sort(),
...
function compactWriteValue(value: unknown, depth = 0): unknown {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    const compact = (key === "description" || key === "notes") && typeof item === "string"
      ? item.length > 240 ? `${item.slice(0, 237)}...` : item
      : compactWriteValue(item, depth + 1);
...
async function createMemory(
  ...
  if (exact) {
    if (exact.project_ids.length > 1) {
      if (sharedMemoryHasChanges(exact, input)) {
        return writeResult({ status: "needs_review", existing_memory_id: exact.id,
          reason: "A shared memory has requested changes; choose an explicit project-safe edit." },
        "create_memory");
      }
      return writeResult({ status: "existing", memory: exact }, "create_memory");
    }
    const updates: Partial<MemoryInput> = {};
    const documentIds = mergeIds(exact.document_ids, input.document_ids);
    const codeIds = mergeIds(exact.code_artifact_ids, input.code_artifact_ids);
    const fileIds = mergeIds(exact.file_ids, input.file_ids);
    if (JSON.stringify(documentIds) !== JSON.stringify(exact.document_ids) && documentIds)
      updates.document_ids = documentIds;
    if (JSON.stringify(codeIds) !== JSON.stringify(exact.code_artifact_ids) && codeIds)
      updates.code_artifact_ids = codeIds;
    if (JSON.stringify(fileIds) !== JSON.stringify(exact.file_ids) && fileIds)
      updates.file_ids = fileIds;
    if (
      input.source_files &&
      JSON.stringify(input.source_files) !== JSON.stringify(exact.source_files)
    )
      updates.source_files = input.source_files;
    if (input.encoding_version && input.encoding_version !== exact.encoding_version)
      updates.encoding_version = input.encoding_version;
    if (Object.keys(updates).length > 0) {
      await beforeMutation(beforeWrite, signal);
      const updated = await knowledge.updateMemory(exact.id, updates, signal);
...
export async function executeKnowledgeWrite(
  ...
  const projectId = currentProject(context, undefined);
  const knowledge = rich(client);
    ...
    for (const field of textFields) {
      if (request[field] !== undefined && request[field] !== memory[field]) {
        return writeResult({ status: "needs_review", memory_id: memory.id,
          reason: "Changed memory claims require supersede_memory to preserve history." }, op);
      }
    }
    await validateAttachments(knowledge, projectId, request, signal);
  ...
  if (op === "supersede_memory") {
    ...
    if (oldMemory.is_obsolete) {
      if (requestedReplacement !== undefined && oldMemory.superseded_by === requestedReplacement)
        return writeResult({ status: "already", old_memory_id: oldMemory.id,
          replacement_memory_id: requestedReplacement }, op);
    ...
    if (currentOld.is_obsolete && currentOld.superseded_by === replacement.id)
      return writeResult({ status: "already", old_memory_id: oldMemory.id,
        replacement_memory_id: replacement.id }, op);
  ...
  if (op === "link_memories") {
    ...
    return writeResult({ status: "linked", memory_id: request.memory_id }, op);
  }
  ...
  if (op === "create_relationship") {
    ...
    if (existing) return writeResult({ status: "existing", relationship: existing }, op);
    await beforeMutation(beforeWrite, signal);
